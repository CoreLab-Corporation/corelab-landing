...
/* ====================================================
   IMPORTS — i18n
==================================================== */
import { applyLang, initI18n, translations, getCurrentLang } from "./i18n.js";
...
/* ====================================================
   LANGUAGE — integrado com i18n.js
==================================================== */
...
function createUserMenu(user) {
  ...
  if (!menu) {
    ...
    document.body.appendChild(menu); // ← anexa ao body, não ao nav
  }
...
document.getElementById("registerSubmit").onclick = async () => {
  ...
  if (!result.success) {
    ...
    // Cadastro bem-sucedido: vai para aba de login com mensagem de boas-vindas
    showAlert("Conta criada! Faça seu login para continuar. 🎉", "success");
    setTimeout(() => {
      document
        .querySelectorAll(".modal-tab-content")
        .forEach((c) => c.classList.remove("active"));
...
/* ====================================================
   SPLASH TAG CYCLE — usa idioma salvo
==================================================== */
const savedLang = (() => {
  try {
    return localStorage.getItem("corelab_lang") || "pt";
  } catch (_) {
    return "pt";
  }
})();
const _t = translations[savedLang] || translations.pt;

const splashTags = [_t.splash_1, _t.splash_2, _t.splash_3];
let tagIndex = 0;
const stagEl = document.getElementById("stag");
if (stagEl) stagEl.textContent = splashTags[0];

const splashInterval = setInterval(() => {
  tagIndex = (tagIndex + 1) % splashTags.length;
  if (stagEl) stagEl.textContent = splashTags[tagIndex];
}, 900);
setTimeout(() => clearInterval(splashInterval), 2800);

/* ====================================================
   I18N — Inicializa idioma salvo (deve ser o ÚLTIMO passo)
==================================================== */
initI18n();
